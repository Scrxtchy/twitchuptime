...
var clientHeaders = new Headers({"Client-ID":'5lc2pznnxzs8gijvw7qgaw8eoisj6nd', "Accept": 'application/vnd.twitchtv.v5+json'})
...
function getChannelId() {
	...
	fetch("https://api.twitch.tv/kraken/search/channels?query=" + channel, { headers: clientHeaders}).then(function (response) {
		response.json().then(function(json){
			getChannelIdCallback(json);
		});
	});
}
...
function getStream() {
	...
	fetch("https://api.twitch.tv/kraken/streams/" + channel, { headers: clientHeaders}).then(function (response) {
		response.json().then(function(json){
			getStreamCallback(json);
		});
	});
...
function upDateText(comment) {
	document.getElementById('uptime').innerHTML = comment;
}
